...
env-resources := ""
# AWS Account Profile
export AWS_PROFILE=inves-technology
...
docker-network-required:
		@if [ ! "$$(docker network ls | grep inves-technology)" ]; then \
			echo "Creating Docker Network inves-technology ..." ;\
			docker network create inves-technology ;\
		else \
			echo "Docker Network inves-technology exists." ;\
		fi
